					...
					<div class="col-lg-4 mt-2">
						<div class="card text-center">
							<div class="card-header">
								<h6 class="m-2 font-weight-bold text-primary">Kamar <?= $kc['no_kamar'] ?></h6>
							</div>
								...
								<div class="card-body">
									...
									<div class="alert alert-info text-center" role="alert">
										<div class="row justify-content-center">
											<!-- <form action="<?= base_url(''); ?>data_kamar/ambilKamarC/<?= $kc['id'] ?>" method="POST">
												<input type="hidden" name="id" value="<?= $kc['id'] ?>">
												<input type="submit" class="btn btn-md btn-primary" value="Check-In">
											</form> -->
											<a href="<?= base_url(''); ?>data_kamar/ambilKamarC/<?= $kc['id'] ?>"
												class="btn btn-md btn-primary" data-toggle="modal"
												data-target="#kamarModal<?= $kc['id']; ?>">Check-In</a>
										</div>
									</div>
									<?php else: ?>
									<div class="alert alert-success text-center" role="alert">
										Kamar Sudah Terisi <strong> <?= $kc['nama_penghuni_kamar'] ?></strong>
										<div class="row justify-content-center">
											<form action="<?= base_url(''); ?>data_kamar/resetKamarc/<?= $kc['id']; ?>" method="POST">
												<input type="hidden" name="id" value="<?= $kc['id']; ?>">
...
<div class="modal fade" id="kamarModal<?= $kc['id']; ?>" tabindex="-1" role="dialog" aria-labelledby="kamarModalLabel"
	aria-hidden="true">
	<div class="modal-dialog" role="document">
		<div class="modal-content">
			...
			<form action="<?= base_url(''); ?>data_kamar/ambilKamarC/<?= $kc['id'] ?>" class="form-group" method="post">
